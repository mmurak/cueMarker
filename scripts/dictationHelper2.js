    ...
    obj1.addEventListener("change",function(evt){
        ...
        case "srt" :
            ...
            reader2.onload = function () {
                G.textArea.value = srtToTxt(reader2.result);
            }
...
function srtToTxt(data) {
    // 00:00:00,000 --> 00:00:05,000  => [[1:9:33.80]]
    let result = '';
    for (let rec of data.split(/\n/)) {
        if (rec.match(/^\d+$/)) continue;
        const m = rec.match(/^(\d{2}):(\d{2}):(\d{2}),(\d{3}) --> \d{2}:\d{2}:\d{2},\d{3}$/);
        if (m) {
            const hh = Number(m[1]);
            const mm = Number(m[2]);
            const ss = Number(m[3]);
